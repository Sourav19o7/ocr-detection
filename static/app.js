...
import { mountSearchResults } from "./components/search-results.js";
...
const routes = [
  ...
  { pattern: /^#\/search\?q=([^&]+)/,      view: (m) => render({ crumbs: [{label:"Batches", href:"#/batches"}, {label:"Search Results"}] }, (el)=>mountSearchResults(el, { query: decodeURIComponent(m[1]) })) },
